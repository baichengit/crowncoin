...
#if defined(NDEBUG)
# error "Crowncoin cannot be compiled without assertions."
#endif
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Crowncoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Crowncoin: find block wanted by given orphan block
...
// Crowncoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Crowncoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Crowncoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Crowncoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Crowncoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Crowncoin: track money supply and mint amount info
...
// Crowncoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Crowncoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Crowncoin: compute stake entropy bit for stake modifier
    ...
    // Crowncoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Crowncoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Crowncoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Crowncoin: attempt to generate suitable proof-of-stake
