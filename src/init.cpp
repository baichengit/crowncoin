...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Crowncoin is shutting down."));
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // Crowncoin: reserve balance amount
    {
    ...
    // requested to kill Crowncoin during the last operation. If so, exit.
